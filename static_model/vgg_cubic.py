...
def make_layers(cfg, batch_norm=False):
    ...
    for v in cfg:
        if v == 'M':
            ...
            if batch_norm:
                if CP:
                    layers += [CubePad(), conv2d, nn.BatchNorm2d(v), nn.ReLU(inplace=True)]
                else:
                    layers += [ZeroPad(), conv2d, nn.BatchNorm2d(v), nn.ReLU(inplace=True)]
                #layers += [conv2d, nn.BatchNorm2d(v), nn.ReLU(inplace=True)]
            else:
                if CP:
                    layers += [CubePad(), conv2d, nn.ReLU(inplace=True)]
                else:
                    layers += [ZeroPad(), conv2d, nn.ReLU(inplace=True)]
