...
def to_3dcube(theta, phi, R):
    ...
    front_trans = np.array(
        [np.ones(phi.shape), np.tan(phi) / np.cos(theta), np.tan(theta)])
    front_trans = np.transpose(front_trans, (1, 2, 0))
    back_trans = np.array(
        [-np.ones(phi.shape), -np.tan(phi) / np.cos(theta), -np.tan(theta)])
    back_trans = np.transpose(back_trans, (1, 2, 0))
    left_trans = np.array(
        [1 / np.tan(theta), np.tan(phi) / np.sin(theta), np.ones(phi.shape)])
    left_trans = np.transpose(left_trans, (1, 2, 0))
    right_trans = np.array(
        [-1 / np.tan(theta), -np.tan(phi) / np.sin(theta), -np.ones(phi.shape)])
    right_trans = np.transpose(right_trans, (1, 2, 0))
    top_trans = np.array(
        [np.cos(theta) / np.tan(phi), np.ones(phi.shape), np.sin(theta) / np.tan(phi)])
    ...
    output_cube[face_conditions(
        theta, phi, 'F'), :] = front_trans[face_conditions(theta, phi, 'F'), :]
    output_cube[face_conditions(
        theta, phi, 'L'), :] = left_trans[face_conditions(theta, phi, 'L'), :]
    output_cube[face_conditions(
        theta, phi, 'B'), :] = back_trans[face_conditions(theta, phi, 'B'), :]
    output_cube[face_conditions(
        theta, phi, 'R'), :] = right_trans[face_conditions(theta, phi, 'R'), :]
    output_cube[face_conditions(
        theta, phi, 'T'), :] = top_trans[face_conditions(theta, phi, 'T'), :]
    output_cube[face_conditions(
        theta, phi, 'D'), :] = down_trans[face_conditions(theta, phi, 'D'), :]

    return output_cube


def set_region_mask(t, p, targ_t, targ_p, l_width, angle):
    print("target: {0},{1} angle={2}".format(
        targ_t, targ_p, angle * 180 / np.pi))
    ...
    if r_most > l_most:
        o_region_mask = (t < r_most) & (t > l_most) & (
...
def myplot(in_name, newsize):
    ...
    norm_resize_im = cv2.normalize(
        _resize_im, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
    bb, gg, rr = cv2.split(norm_resize_im)
    ...
    x, y, z = to_3dsphere(theta, phi, rr)
    cube_mesh = to_3dcube(theta, phi, 1)

    region_mask = solid_region_mask(
        theta, phi, targ_th / 180.0 * np.pi, targ_ph / 180.0 * np.pi, 0.05, vang_x / 180.0 * np.pi)
    if VIS_MASK:
    ...
    ax_sp.plot_wireframe(
        cube_mesh[:, :, 0], cube_mesh[:, :, 1], cube_mesh[:, :, 2], rstride=10, cstride=10)
    ...
    if(BOX):
        ...
        ax_sp.plot_wireframe(cube_mesh[:, :, 0], cube_mesh[:, :, 1],
                             cube_mesh[:, :, 2], color='r', rstride=6, cstride=6)
        # ax_sp.plot_surface(cube_mesh[:,:,0], cube_mesh[:,:,1], cube_mesh[:,:,2], color='r',
        #                    antialiased=False, linewidth=0, shade=False)

    if WRITE_FIG:
        plt.savefig(in_name.split('.')[0] + '_cs' +
                    str(view_th) + '_' + str(view_ph) + '.jpg')
...
def cubeplot(in_dir, view_th, view_ph):
    ...
    for idx in range(6):
        ...
        tmp_im = cv2.normalize(cv2.resize(raw_tmp, (int(raw_tmp.shape[0] * scale_c), int(
            raw_tmp.shape[0] * scale_c))), norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
        bb, gg, rr = cv2.split(tmp_im)
...
def main():
    source_path = '/[Path to CP image output]/'
    main_size = (2000, 1000)  # Out-box size
    sub_size = (1000, 500)   # In-box size
    view_th = 160            # Theta angle
    view_ph = 55              # Phi angle

    myplot('/[Input image].jpg', sub_size)
    plt.show()
    ...
    myplot('/[Input image].jpg', main_size)
